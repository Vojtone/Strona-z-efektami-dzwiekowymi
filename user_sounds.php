<?php
    ...
    {
        header('Location: user_sounds.php?category=wszystkie&sort_by=najnowsze');
        exit();
    }

    if (!isset($_GET['page']))
    {
        header('Location: user_sounds.php?category='.$_GET['category'].'&sort_by='.$_GET['sort_by'].'&page=1');
        exit();
    }
?>
        ...
        <div id=content>
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if ($_GET['sort_by']=='najnowsze')
                            {
                                ...
                                {
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id ORDER BY addition_date DESC");
                                }
                                else
                                {
                                    $category = $_GET['category'];
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id AND category='$category' ORDER BY addition_date DESC");
                                }
                            }
                            if ($_GET['sort_by']=='najstarsze')
                            {
                                if ($_GET['category']=='wszystkie')
                                {
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id ORDER BY addition_date ASC");
                                }
                                else
                                {
                                    $category = $_GET['category'];
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id AND category='$category' ORDER BY addition_date ASC");
                                }
                            }
                            if ($_GET['sort_by']=='najwiecej_pobran')
                            {
                                if ($_GET['category']=='wszystkie')
                                {
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id ORDER BY download_counter DESC");
                                }
                                else
                                {
                                    $category = $_GET['category'];
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id AND category='$category' ORDER BY download_counter DESC");
                                }
                            }
                            if ($_GET['sort_by']=='najlepsza_ocena')
                            {
                                if ($_GET['category']=='wszystkie')
                                {
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id ORDER BY rating DESC");
                                }
                                else
                                {
                                    $category = $_GET['category'];
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id AND category='$category' ORDER BY rating DESC");
                                }
                            }
                            if ($_GET['sort_by']=='najwiecej_ocen')
                            {
                                if ($_GET['category']=='wszystkie')
                                {
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id ORDER BY rating_counter DESC");
                                }
                                else
                                {
                                    $category = $_GET['category'];
                                    $request_result = $connection->query("SELECT * FROM sounds WHERE user_id=$user_id AND category='$category' ORDER BY rating_counter DESC");
                                }
                            }
                            echo 'Twoje dźwięki <br />';
                            ...
                            for ($i=0; $i<$how_many_rows; $i++)
                            {
                                ...
                                if ( $i>=(($_GET['page']-1)*$how_many_sounds_on_page) && ($i<$_GET['page']*10) && ($i<$how_many_rows) ) //wyswietlanie wybranych do page
                                {
                                    ...
                                    echo '<div style="float: left; margin-left: 5px;"><form action="delete_sound.php" method="post"><input src="img/delete.png" title="Usuń dźwięk" width="40" height="40" type="image" value="'.$row['id'].'" name="delete_sound_button"></input></form></div>';
                                    echo '</div>';
                            ...
                            if ($_GET['page'] > 1)
                            {
                                ...
                                echo '<a href="user_sounds.php?category='.$_GET['category'].'&sort_by='.$_GET['sort_by'].'&page='.$previous.'" ><div class="page"><<</div></a>';
                            }
                            ...
                            for ($i = $page-2; $i <= $page+2 ; $i++)
                            {
                                if ($i>0 && $i<=round($how_many_pages+0.49))
                                {
                                    if ($_GET['page'] == $i)
                                    {
                                        echo '<a href="user_sounds.php?category='.$_GET['category'].'&sort_by='.$_GET['sort_by'].'&page='.$i.'" ><div class="page" style="color: #ff6d0b;">'.$i.'</div></a>';
                                    }
                                    else
                                    {
                                        echo '<a href="user_sounds.php?category='.$_GET['category'].'&sort_by='.$_GET['sort_by'].'&page='.$i.'" ><div class="page">'.$i.'</div></a>';
                                    }
                                }
                            }
                            ...
                            {
                                $next = $_GET['page'] + 1;
                                echo '<a href="user_sounds.php?category='.$_GET['category'].'&sort_by='.$_GET['sort_by'].'&page='.$next.'" ><div class="page">>></div></a>';
                            }
                            $connection->close();
                        }
            ...
            <div id="content_list">
                ...
                <ol style="margin-top: 20px;">
                    <?php
                        echo
                        '<a href="user_sounds.php?category=wszystkie&sort_by='.$_GET['sort_by'].'" ><li>Wszystkie</li></a>
                        <a href="user_sounds.php?category=natura&sort_by='.$_GET['sort_by'].'" ><li>Natura</li></a>
                        <a href="user_sounds.php?category=akcja&sort_by='.$_GET['sort_by'].'" ><li>Akcja</li></a>
                        <a href="user_sounds.php?category=obyczajowe&sort_by='.$_GET['sort_by'].'" ><li>Obyczajowe</li></a>
                        <a href="user_sounds.php?category=inne&sort_by='.$_GET['sort_by'].'" ><li>Inne</li></a>';
                    ?>
				</ol>
                </div>
                ...
                <ol style="margin-top: 20px;">
                    <?php
                        echo
                        '<a href="user_sounds.php?category='.$_GET['category'].'&sort_by=najnowsze" ><li>Najnowsze</li></a>
                        <a href="user_sounds.php?category='.$_GET['category'].'&sort_by=najstarsze" ><li>Najstarsze</li></a>
                        <a href="user_sounds.php?category='.$_GET['category'].'&sort_by=najwiecej_pobran" ><li>Najwięcej pobrań</li></a>
                        <a href="user_sounds.php?category='.$_GET['category'].'&sort_by=najlepsza_ocena" ><li>Najlepsza ocena</li></a>
                        <a href="user_sounds.php?category='.$_GET['category'].'&sort_by=najwiecej_ocen" ><li>Najwięcej ocen</li></a>';
                    ?>
                </ol>
                </div>
            </div>
